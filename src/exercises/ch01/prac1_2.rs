...
mod tests {
    ...
    fn test_checkout_with_promotion() {
        ...
        let expected_price =
            price_info.get(&Items::Milk).unwrap() * 2 + price_info.get(&Items::Fish).unwrap();
        assert_eq!(result.unwrap(), expected_price);
    }

    #[test]
    fn test_checkout_without_promotion() {
        let price_info: HashMap<Items, i64> =
            HashMap::from([(Items::Milk, 1500), (Items::Fish, 5000)]);
    ...
    fn test_checkout_with_non_existing_item() {
        let price_info = HashMap::from([(Items::Milk, 1500)]);
